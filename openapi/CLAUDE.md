...
A **magic HTTP body** argument (`httpRequestBody`/`httpResponseBody`) has no description at the body level. It references a struct type reflected as a whole, so the struct's *fields* are described by their `jsonschema_description` tags (as above) - but the struct's own godoc does **not** reach the rendered document: the reflector runs without `AddGoComments`, so no Go comments are read at runtime. The body-level description is therefore empty; document a magic body through its field tags and the endpoint's own godoc (the operation description). A magic body over a non-struct type has neither and simply goes undescribed, which is acceptable.
