...
func TestMarshalJSON_ParamDescriptions(t *testing.T) {
	t.Parallel()
	assert := testarossa.For(t)
...
Output:
  ...
  - confidence: Model confidence score`,
	...
	outSchema := resolveSchema(schemas, "Forecast_OUT")
...
func TestMarshalJSON_ParamDescriptions_POST(t *testing.T) {
	t.Parallel()
	assert := testarossa.For(t)
...
Output:
  - id: The generated user ID`,
	...
	inSchema := resolveSchema(schemas, "Create_IN")
	...
	outSchema := resolveSchema(schemas, "Create_OUT")
	outProps := outSchema["properties"].(map[string]any)
	idProp := outProps["id"].(map[string]any)
	assert.Expect(idProp["description"], "The generated user ID")
}

func TestMarshalJSON_JsonSchemaTags(t *testing.T) {
	t.Parallel()
	assert := testarossa.For(t)
	...
	components := doc["components"].(map[string]any)
	schemas := components["schemas"].(map[string]any)
	locSchema := schemas["Search_IN_Location"].(map[string]any)
