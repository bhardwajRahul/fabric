...
func TestConnector_TracingExport(t *testing.T) {
	...
	// Nothing traced yet
	assert.Zero(int(ts.insertionPoint.Load()))
	assert.Zero(countExported)
	assert.Zero(len(ts.selected1) + len(ts.selected2))
	assert.Zero(ts.lockCount)
	...
	assert.Equal(span.SpanContext().TraceID(), subSpan1.SpanContext().TraceID())
	subSpan1.End()

	// The spans should be buffered but not yet exported
	assert.Equal(3, int(ts.insertionPoint.Load()))
	assert.Zero(countExported)
	assert.Zero(ts.lockCount)
	...
	assert.Equal(1, len(ts.selected1)+len(ts.selected2))

	// The closed subspan should have gotten immediately exported
	assert.Equal(3, int(ts.insertionPoint.Load()))
	assert.Equal(1, countExported)
	assert.True(exportedSpans[subSpan1.SpanContext().SpanID().String()])
	assert.Equal(1, ts.lockCount)

	// Add a second subspan
	_, subSpan2 := tracer.Start(subCtx, "3.2")
	assert.Equal(span.SpanContext().TraceID(), subSpan2.SpanContext().TraceID())
	subSpan2.End()
	ts.ForceFlush(ctx) // Flush the queue

	// The new subspan should have gotten immediately exported and not buffered
	assert.Equal(3, int(ts.insertionPoint.Load()))
	assert.Equal(2, countExported)
	assert.True(exportedSpans[subSpan2.SpanContext().SpanID().String()])
	assert.Equal(2, ts.lockCount)
	...
	assert.Equal(3, int(ts.insertionPoint.Load()))
	assert.Equal(3, countExported)
	assert.True(exportedSpans[span.SpanContext().SpanID().String()])
	assert.Equal(3, ts.lockCount)
...
func TestConnector_TracingTTLClearMaps(t *testing.T) {
	...
	assert.Equal(1, ts.lockCount)
	ts.Select("2")
	assert.Equal(2, len(ts.selected1)+len(ts.selected2))
	assert.Equal(2, ts.lockCount)
	ts.Select("2")
	assert.Equal(2, len(ts.selected1)+len(ts.selected2))
	assert.Equal(3, ts.lockCount)
	ts.Select("3")
	assert.Equal(3, len(ts.selected1)+len(ts.selected2))
	assert.Equal(4, ts.lockCount)
...
func TestConnector_TracingSelectorCapacityRollover(t *testing.T) {
	...
	assert.Len(ts.selected1, maxSelected/2)

	ts.Select(strconv.Itoa(maxSelected / 2))
	assert.Len(ts.selected1, 1)
	assert.Len(ts.selected2, maxSelected/2)
...
func TestConnector_TracingBufferCapacityRollover(t *testing.T) {
	...
	// Fill in the buffer
	assert.Zero(ts.insertionPoint.Load())
	for i := range n {
		assert.Equal(int32(i), ts.insertionPoint.Load())
		assert.Nil(ts.buffer[i].Load())
		_, span := tracer.Start(ctx, "A")
		span.End()
		assert.NotNil(ts.buffer[i].Load())
		assert.Equal(int32(i+1), ts.insertionPoint.Load())
	}
