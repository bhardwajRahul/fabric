...
/*
Sharded creates and runs the Sharded workflow, blocking until termination.
*/
func (_c Executor) Sharded(ctx context.Context, tag string, delayMs int) (recorded bool, status string, err error) { // MARKER: Sharded
	if _c.runner == nil {
		return recorded, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out ShardedOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Sharded.URL(), ShardedIn{
		Tag:     tag,
		DelayMs: delayMs,
	}, &out)
