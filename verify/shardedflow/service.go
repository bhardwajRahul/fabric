...
/*
Service implements shardedflow.verify, exercising cross-shard global priority+fairness aggregation with NumShards>1.
*/
type Service struct {
...
func (svc *Service) Record(ctx context.Context, flow *workflow.Flow) (recorded bool, err error) { // MARKER: Record
	var in shardedflowapi.RecordIn
	flow.ParseState(&in)
	if in.DelayMs > 0 {
...
/*
Sharded defines the single-task graph (record -> END) used to observe cross-shard global dispatch order.
