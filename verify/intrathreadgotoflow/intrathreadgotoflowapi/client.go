...
*/
func (_c Executor) TaskA(ctx context.Context, target int) (targetOut int, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{Target: target}, &out, _c.inFlow, _c.outFlow)
	return out.TargetOut, err
}

/*
LoopTask creates and runs the LoopTask task.
*/
func (_c Executor) LoopTask(ctx context.Context, loops, target int) (loopsOut int, err error) { // MARKER: LoopTask
	var out LoopTaskOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, LoopTask.Method, LoopTask.Route, LoopTaskIn{Loops: loops, Target: target}, &out, _c.inFlow, _c.outFlow)
	return out.LoopsOut, err
}

/*
NormalC creates and runs the NormalC task.
*/
func (_c Executor) NormalC(ctx context.Context) (stamp string, err error) { // MARKER: NormalC
	var out NormalCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, NormalC.Method, NormalC.Route, NormalCIn{}, &out, _c.inFlow, _c.outFlow)
...
*/
func (_c Executor) TaskD(ctx context.Context, loops int, stamp string) (finalResult string, err error) { // MARKER: TaskD
	var out TaskDOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskD.Method, TaskD.Route, TaskDIn{Loops: loops, Stamp: stamp}, &out, _c.inFlow, _c.outFlow)
	return out.FinalResult, err
}

/*
IntraThreadGoto creates and runs the IntraThreadGoto workflow.
*/
func (_c Executor) IntraThreadGoto(ctx context.Context, target int) (finalResult string, status string, err error) { // MARKER: IntraThreadGoto
	if _c.runner == nil {
		return finalResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out IntraThreadGotoOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, IntraThreadGoto.URL(), IntraThreadGotoIn{Target: target}, &out)
