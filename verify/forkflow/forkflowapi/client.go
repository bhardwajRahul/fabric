...
*/
func (_c Executor) TaskA(ctx context.Context, value int) (valueOut int, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{Value: value}, &out, _c.inFlow, _c.outFlow)
	return out.ValueOut, err
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, value int) (valueOut int, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{Value: value}, &out, _c.inFlow, _c.outFlow)
	return out.ValueOut, err
}
...
*/
func (_c Executor) TaskC(ctx context.Context, value int) (valueOut int, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{Value: value}, &out, _c.inFlow, _c.outFlow)
	return out.ValueOut, err
}

/*
Pipe creates and runs the Pipe workflow.
*/
func (_c Executor) Pipe(ctx context.Context, value int) (finalValue int, status string, err error) { // MARKER: Pipe
	if _c.runner == nil {
		return finalValue, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out PipeOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Pipe.URL(), PipeIn{Value: value}, &out)
	return out.ValueOut, status, err
}
