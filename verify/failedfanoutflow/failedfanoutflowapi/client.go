...
/*
Src creates and runs the Src task.
*/
func (_c Executor) Src(ctx context.Context) (started bool, err error) { // MARKER: Src
	var out SrcOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Src.Method, Src.Route, SrcIn{}, &out, _c.inFlow, _c.outFlow)
...
/*
FailedFanOut creates and runs the FailedFanOut workflow, blocking until termination.
*/
func (_c Executor) FailedFanOut(ctx context.Context) (sumExecuted int, totalExecuted int, status string, err error) { // MARKER: FailedFanOut
	if _c.runner == nil {
		return sumExecuted, totalExecuted, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out FailedFanOutOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, FailedFanOut.URL(), FailedFanOutIn{}, &out)
