...
func (_c Executor) Bounded(ctx context.Context, tag string) (tallied bool, err error) { // MARKER: Bounded
	var out BoundedOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Bounded.Method, Bounded.Route, BoundedIn{
		Tag: tag,
...
/*
MaxConcurrency creates and runs the MaxConcurrency workflow, blocking until termination.
*/
func (_c Executor) MaxConcurrency(ctx context.Context, tag string) (tallied bool, status string, err error) { // MARKER: MaxConcurrency
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out MaxConcurrencyOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, MaxConcurrency.URL(), MaxConcurrencyIn{
		Tag: tag,
	}, &out)
