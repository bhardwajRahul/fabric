...
/*
Bounded creates and runs the Bounded task.
*/
func (_c Executor) Bounded(ctx context.Context, tag string) (tallied bool, err error) { // MARKER: Bounded
	var out BoundedOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Bounded.Method, Bounded.Route, BoundedIn{Tag: tag}, &out, _c.inFlow, _c.outFlow)
	return out.Tallied, err // No trace
}

/*
Open creates and runs the Open task.
*/
func (_c Executor) Open(ctx context.Context, tag string) (tallied bool, err error) { // MARKER: Open
	var out OpenOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Open.Method, Open.Route, OpenIn{Tag: tag}, &out, _c.inFlow, _c.outFlow)
	return out.Tallied, err // No trace
}

/*
SaturatedBand creates and runs the SaturatedBand workflow.
*/
func (_c Executor) SaturatedBand(ctx context.Context, tag string) (tallied bool, status string, err error) { // MARKER: SaturatedBand
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out SaturatedBandOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, SaturatedBand.URL(), SaturatedBandIn{Tag: tag}, &out)
	return out.Tallied, status, err
}

/*
OpenBand creates and runs the OpenBand workflow.
*/
func (_c Executor) OpenBand(ctx context.Context, tag string) (tallied bool, status string, err error) { // MARKER: OpenBand
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out OpenBandOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, OpenBand.URL(), OpenBandIn{Tag: tag}, &out)
	return out.Tallied, status, err
}
