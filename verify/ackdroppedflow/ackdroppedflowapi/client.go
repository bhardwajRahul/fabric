...
// Park creates and runs the Park task.
func (_c Executor) Park(ctx context.Context, tag string) (parked bool, err error) { // MARKER: Park
	var out ParkOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Park.Method, Park.Route, ParkIn{
		Tag: tag,
	}, &out, _c.inFlow, _c.outFlow)
	return out.Parked, err // No trace
}

// Ping creates and runs the Ping task.
func (_c Executor) Ping(ctx context.Context, tag string) (pinged bool, err error) { // MARKER: Ping
	var out PingOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Ping.Method, Ping.Route, PingIn{
		Tag: tag,
	}, &out, _c.inFlow, _c.outFlow)
	return out.Pinged, err // No trace
}

// AckDropped creates and runs the AckDropped workflow, blocking until termination.
func (_c Executor) AckDropped(ctx context.Context, tag string) (parked bool, status string, err error) { // MARKER: AckDropped
	if _c.runner == nil {
		return parked, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out AckDroppedOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, AckDropped.URL(), AckDroppedIn{
		Tag: tag,
	}, &out)
	return out.Parked, status, err
}

// Echo creates and runs the Echo workflow, blocking until termination.
func (_c Executor) Echo(ctx context.Context, tag string) (pinged bool, status string, err error) { // MARKER: Echo
	if _c.runner == nil {
		return pinged, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out EchoOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Echo.URL(), EchoIn{
		Tag: tag,
	}, &out)
	return out.Pinged, status, err
}
