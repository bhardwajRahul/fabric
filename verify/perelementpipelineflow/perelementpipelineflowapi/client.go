...
/*
TaskS creates and runs the TaskS task.
*/
func (_c Executor) TaskS(ctx context.Context, items []string) (itemsOut []string, err error) { // MARKER: TaskS
	var out TaskSOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskS.Method, TaskS.Route, TaskSIn{Items: items}, &out, _c.inFlow, _c.outFlow)
	return out.ItemsOut, err
}

/*
TaskH creates and runs the TaskH task.
*/
func (_c Executor) TaskH(ctx context.Context, item string) (itemUpper string, err error) { // MARKER: TaskH
	var out TaskHOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskH.Method, TaskH.Route, TaskHIn{Item: item}, &out, _c.inFlow, _c.outFlow)
	return out.ItemUpper, err
}

/*
TaskA creates and runs the TaskA task.
*/
func (_c Executor) TaskA(ctx context.Context, itemUpper string) (aProcessed string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{ItemUpper: itemUpper}, &out, _c.inFlow, _c.outFlow)
	return out.AProcessed, err
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, itemUpper string) (bProcessed string, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{ItemUpper: itemUpper}, &out, _c.inFlow, _c.outFlow)
	return out.BProcessed, err
}
...
/*
PerElementPipeline creates and runs the PerElementPipeline workflow.
*/
func (_c Executor) PerElementPipeline(ctx context.Context, items []string) (finalCount int, status string, err error) { // MARKER: PerElementPipeline
	if _c.runner == nil {
		return finalCount, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out PerElementPipelineOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, PerElementPipeline.URL(), PerElementPipelineIn{Items: items}, &out)
	return out.FinalCount, status, err
}
