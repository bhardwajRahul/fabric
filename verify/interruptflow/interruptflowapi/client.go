...
*/
func (_c Executor) TaskA(ctx context.Context, prompt string) (promptOut string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{Prompt: prompt}, &out, _c.inFlow, _c.outFlow)
	return out.PromptOut, err
}

/*
AwaitInput creates and runs the AwaitInput task.
*/
func (_c Executor) AwaitInput(ctx context.Context, userInput string) (userInputOut string, err error) { // MARKER: AwaitInput
	var out AwaitInputOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, AwaitInput.Method, AwaitInput.Route, AwaitInputIn{UserInput: userInput}, &out, _c.inFlow, _c.outFlow)
	return out.UserInputOut, err
}

/*
Compose creates and runs the Compose task.
*/
func (_c Executor) Compose(ctx context.Context, prompt, userInput string) (result string, err error) { // MARKER: Compose
	var out ComposeOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Compose.Method, Compose.Route, ComposeIn{Prompt: prompt, UserInput: userInput}, &out, _c.inFlow, _c.outFlow)
	return out.Result, err
}

/*
Interruptor creates and runs the Interruptor workflow.
*/
func (_c Executor) Interruptor(ctx context.Context, prompt string) (result string, status string, err error) { // MARKER: Interruptor
	if _c.runner == nil {
		return result, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out InterruptorOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Interruptor.URL(), InterruptorIn{Prompt: prompt}, &out)
