...
*/
func (_c Executor) TaskA(ctx context.Context) (stepA bool, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{}, &out, _c.inFlow, _c.outFlow)
	return out.StepA, err
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, stepA bool) (stepB bool, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{StepA: stepA}, &out, _c.inFlow, _c.outFlow)
	return out.StepB, err
}

/*
TaskC creates and runs the TaskC task.
*/
func (_c Executor) TaskC(ctx context.Context, stepB bool) (stepC bool, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{StepB: stepB}, &out, _c.inFlow, _c.outFlow)
	return out.StepC, err
}

/*
Breakpoint creates and runs the Breakpoint workflow.
*/
func (_c Executor) Breakpoint(ctx context.Context) (stepC bool, status string, err error) { // MARKER: Breakpoint
	if _c.runner == nil {
		return stepC, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out BreakpointOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Breakpoint.URL(), BreakpointIn{}, &out)
	return out.StepC, status, err
}
