...
/*
Flaky creates and runs the Flaky task.
*/
func (_c Executor) Flaky(ctx context.Context, attempts, target int) (attemptsOut int, err error) { // MARKER: Flaky
	var out FlakyOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Flaky.Method, Flaky.Route, FlakyIn{Attempts: attempts, Target: target}, &out, _c.inFlow, _c.outFlow)
...
*/
func (_c Executor) TaskB(ctx context.Context, attempts int) (finalAttempts int, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{Attempts: attempts}, &out, _c.inFlow, _c.outFlow)
	return out.FinalAttempts, err
}

/*
Retry creates and runs the Retry workflow.
*/
func (_c Executor) Retry(ctx context.Context, target int) (finalAttempts int, status string, err error) { // MARKER: Retry
	if _c.runner == nil {
		return finalAttempts, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out RetryOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Retry.URL(), RetryIn{Target: target}, &out)
