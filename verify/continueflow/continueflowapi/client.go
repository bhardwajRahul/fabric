...
*/
func (_c Executor) Increment(ctx context.Context, counter int) (counterOut int, err error) { // MARKER: Increment
	var out IncrementOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Increment.Method, Increment.Route, IncrementIn{Counter: counter}, &out, _c.inFlow, _c.outFlow)
	return out.CounterOut, err
}

/*
Counting creates and runs the Counting workflow.
*/
func (_c Executor) Counting(ctx context.Context, counter int) (finalCounter int, status string, err error) { // MARKER: Counting
	if _c.runner == nil {
		return finalCounter, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out CountingOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Counting.URL(), CountingIn{Counter: counter}, &out)
	return out.CounterOut, status, err
}
