...
/*
Reducer creates and runs the Reducer workflow.
*/
func (_c Executor) Reducer(ctx context.Context) (finalSum int, finalList, finalSet []string, status string, err error) { // MARKER: Reducer
	if _c.runner == nil {
		return finalSum, finalList, finalSet, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out ReducerOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Reducer.URL(), ReducerIn{}, &out)
	return out.FinalSum, out.FinalList, out.FinalSet, status, err
}
