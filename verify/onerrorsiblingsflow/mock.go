...
type Mock struct {
	*Intermediate
	...
	mockTaskE            func(ctx context.Context, flow *workflow.Flow, handled bool, markB bool, markC bool, markD bool) (recovered, siblingsRan bool, err error) // MARKER: TaskE
...
func (svc *Mock) MockTaskE(handler func(ctx context.Context, flow *workflow.Flow, handled bool, markB bool, markC bool, markD bool) (recovered, siblingsRan bool, err error)) *Mock { // MARKER: TaskE
	svc.mockTaskE = handler
	return svc
}

// TaskE executes the mock handler.
func (svc *Mock) TaskE(ctx context.Context, flow *workflow.Flow, handled bool, markB bool, markC bool, markD bool) (recovered, siblingsRan bool, err error) { // MARKER: TaskE
	if svc.mockTaskE != nil {
		recovered, siblingsRan, err = svc.mockTaskE(ctx, flow, handled, markB, markC, markD)
	}
	return recovered, siblingsRan, errors.Trace(err)
}
...
func (svc *Mock) MockFanOutError(handler func(ctx context.Context, flow *workflow.Flow) (recovered bool, err error)) *Mock { // MARKER: FanOutError
	...
	svc.mockFanOutErrorGraph = func(ctx context.Context) (graph *workflow.Graph, err error) {
		g := workflow.NewGraph(onerrorsiblingsflowapi.FanOutError.URL())
	...
	err := svc.Subscribe(mockName, func(w http.ResponseWriter, r *http.Request) error {
		...
		snap := f.Snapshot()
		var in onerrorsiblingsflowapi.FanOutErrorIn
		f.ParseState(&in)
		recovered, err := handler(r.Context(), &f)
		if err != nil {
			return err // No trace
		}
		out := onerrorsiblingsflowapi.FanOutErrorOut{Recovered: recovered}
		f.SetChanges(out, snap)
		w.Header().Set("Content-Type", "application/json")
		return json.NewEncoder(w).Encode(&f)
	},
		sub.At("POST", mockRoute),
		sub.Task(onerrorsiblingsflowapi.FanOutErrorIn{}, onerrorsiblingsflowapi.FanOutErrorOut{}),
	)
	if err == nil {
