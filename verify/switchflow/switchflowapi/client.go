...
/*
Router creates and runs the Router task.
*/
func (_c Executor) Router(ctx context.Context, amount int) (amountOut int, err error) { // MARKER: Router
	var out RouterOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Router.Method, Router.Route, RouterIn{Amount: amount}, &out, _c.inFlow, _c.outFlow)
	return out.AmountOut, err
}

/*
HandleHigh creates and runs the HandleHigh task.
*/
func (_c Executor) HandleHigh(ctx context.Context, amount int) (branch string, err error) { // MARKER: HandleHigh
	var out HandleHighOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, HandleHigh.Method, HandleHigh.Route, HandleHighIn{Amount: amount}, &out, _c.inFlow, _c.outFlow)
	return out.Branch, err
}

/*
HandleMid creates and runs the HandleMid task.
*/
func (_c Executor) HandleMid(ctx context.Context, amount int) (branch string, err error) { // MARKER: HandleMid
	var out HandleMidOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, HandleMid.Method, HandleMid.Route, HandleMidIn{Amount: amount}, &out, _c.inFlow, _c.outFlow)
	return out.Branch, err
}

/*
HandleLow creates and runs the HandleLow task.
*/
func (_c Executor) HandleLow(ctx context.Context, amount int) (branch string, err error) { // MARKER: HandleLow
	var out HandleLowOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, HandleLow.Method, HandleLow.Route, HandleLowIn{Amount: amount}, &out, _c.inFlow, _c.outFlow)
	return out.Branch, err
}

/*
Switch creates and runs the Switch workflow, blocking until termination.
*/
func (_c Executor) Switch(ctx context.Context, amount int) (branch string, status string, err error) { // MARKER: Switch
	if _c.runner == nil {
		return branch, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out SwitchOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Switch.URL(), SwitchIn{Amount: amount}, &out)
	return out.Branch, status, err
}

/*
SwitchNoMatch creates and runs the SwitchNoMatch workflow, blocking until termination.
*/
func (_c Executor) SwitchNoMatch(ctx context.Context, amount int) (branch string, status string, err error) { // MARKER: SwitchNoMatch
	if _c.runner == nil {
		return branch, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out SwitchNoMatchOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, SwitchNoMatch.URL(), SwitchNoMatchIn{Amount: amount}, &out)
	return out.Branch, status, err
}
