...
/*
Parent creates and runs the Parent task.
*/
func (_c Executor) Parent(ctx context.Context, value int, innerDone bool, innerResult int) (parentResult string, err error) { // MARKER: Parent
	var out ParentOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Parent.Method, Parent.Route, ParentIn{Value: value, InnerDone: innerDone, InnerResult: innerResult}, &out, _c.inFlow, _c.outFlow)
	return out.ParentResult, err
}

/*
InnerA creates and runs the InnerA task.
*/
func (_c Executor) InnerA(ctx context.Context, value int) (innerStage int, err error) { // MARKER: InnerA
	var out InnerAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, InnerA.Method, InnerA.Route, InnerAIn{Value: value}, &out, _c.inFlow, _c.outFlow)
	return out.InnerStage, err
}

/*
InnerB creates and runs the InnerB task.
*/
func (_c Executor) InnerB(ctx context.Context, innerStage int) (innerResult int, innerDone bool, err error) { // MARKER: InnerB
	var out InnerBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, InnerB.Method, InnerB.Route, InnerBIn{InnerStage: innerStage}, &out, _c.inFlow, _c.outFlow)
	return out.InnerResult, out.InnerDone, err
}

/*
Inner creates and runs the Inner workflow.
*/
func (_c Executor) Inner(ctx context.Context, value int) (innerResult int, innerDone bool, status string, err error) { // MARKER: Inner
	if _c.runner == nil {
		return innerResult, innerDone, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out InnerOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Inner.URL(), InnerIn{Value: value}, &out)
	return out.InnerResult, out.InnerDone, status, err
}

/*
DynamicSubgraph creates and runs the DynamicSubgraph workflow.
*/
func (_c Executor) DynamicSubgraph(ctx context.Context, value int) (parentResult string, status string, err error) { // MARKER: DynamicSubgraph
	if _c.runner == nil {
		return parentResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out DynamicSubgraphOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, DynamicSubgraph.URL(), DynamicSubgraphIn{Value: value}, &out)
	return out.ParentResult, status, err
}
