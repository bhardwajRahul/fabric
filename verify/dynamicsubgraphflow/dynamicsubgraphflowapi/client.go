...
func (_c Executor) Inner(ctx context.Context, value int) (innerResult int, innerDone bool, status string, err error) { // MARKER: Inner
	...
	var out InnerOut
	status, err = marshalWorkflow(ctx, _c.runner, Inner.URL(), InnerIn{Value: value}, &out)
...
func (_c Executor) DynamicSubgraph(ctx context.Context, value int) (parentResult string, status string, err error) { // MARKER: DynamicSubgraph
	...
	status, err = marshalWorkflow(ctx, _c.runner, DynamicSubgraph.URL(), DynamicSubgraphIn{Value: value}, &out)
