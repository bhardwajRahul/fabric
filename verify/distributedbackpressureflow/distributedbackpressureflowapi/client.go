package distributedbackpressureflowapi
...
/*
DistributedBackpressure creates and runs the DistributedBackpressure workflow.
*/
func (_c Executor) DistributedBackpressure(ctx context.Context, tag string) (tallied bool, status string, err error) { // MARKER: DistributedBackpressure
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out DistributedBackpressureOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, DistributedBackpressure.URL(), DistributedBackpressureIn{Tag: tag}, &out)
