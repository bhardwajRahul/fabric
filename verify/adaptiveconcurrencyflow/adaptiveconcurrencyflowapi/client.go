...
/*
Adaptive creates and runs the Adaptive task.
*/
func (_c Executor) Adaptive(ctx context.Context, tag string) (tallied bool, err error) { // MARKER: Adaptive
	var out AdaptiveOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Adaptive.Method, Adaptive.Route, AdaptiveIn{Tag: tag}, &out, _c.inFlow, _c.outFlow)
	return out.Tallied, err // No trace
}

/*
AdaptiveConcurrency creates and runs the AdaptiveConcurrency workflow.
*/
func (_c Executor) AdaptiveConcurrency(ctx context.Context, tag string) (tallied bool, status string, err error) { // MARKER: AdaptiveConcurrency
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out AdaptiveConcurrencyOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, AdaptiveConcurrency.URL(), AdaptiveConcurrencyIn{Tag: tag}, &out)
