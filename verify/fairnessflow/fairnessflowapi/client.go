...
/*
Tally creates and runs the Tally task.
*/
func (_c Executor) Tally(ctx context.Context, tag string, delayMs int) (tallied bool, err error) { // MARKER: Tally
	var out TallyOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Tally.Method, Tally.Route, TallyIn{
		Tag:     tag,
		DelayMs: delayMs,
...
/*
Fairness creates and runs the Fairness workflow, blocking until termination.
*/
func (_c Executor) Fairness(ctx context.Context, tag string, delayMs int) (tallied bool, status string, err error) { // MARKER: Fairness
	if _c.runner == nil {
		return tallied, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out FairnessOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Fairness.URL(), FairnessIn{
		Tag:     tag,
		DelayMs: delayMs,
	}, &out)
