...
/*
Slow creates and runs the Slow task.
*/
func (_c Executor) Slow(ctx context.Context) (done bool, err error) { // MARKER: Slow
	var out SlowOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Slow.Method, Slow.Route, SlowIn{}, &out, _c.inFlow, _c.outFlow)
	return out.Done, err
}

/*
TimeBudget creates and runs the TimeBudget workflow.
*/
func (_c Executor) TimeBudget(ctx context.Context) (done bool, status string, err error) { // MARKER: TimeBudget
	if _c.runner == nil {
		return done, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out TimeBudgetOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, TimeBudget.URL(), TimeBudgetIn{}, &out)
