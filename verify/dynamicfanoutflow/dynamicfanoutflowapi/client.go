...
*/
func (_c Executor) TaskA(ctx context.Context, items []string) (itemsOut []string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{Items: items}, &out, _c.inFlow, _c.outFlow)
	return out.ItemsOut, err // No trace
}
...
func (_c Executor) DynamicFanOut(ctx context.Context, items []string) (processedCount int, status string, err error) { // MARKER: DynamicFanOut
	...
	status, err = marshalWorkflow(ctx, _c.runner, DynamicFanOut.URL(), DynamicFanOutIn{Items: items}, &out)
