...
*/
func (_c Executor) TaskA(ctx context.Context, trigger string) (triggerOut string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{Trigger: trigger}, &out, _c.inFlow, _c.outFlow)
	return out.TriggerOut, err // No trace
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, trigger string) (result string, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{Trigger: trigger}, &out, _c.inFlow, _c.outFlow)
	return out.Result, err // No trace
}

/*
Handler creates and runs the Handler task.
*/
func (_c Executor) Handler(ctx context.Context, onErr *errors.TracedError) (result string, err error) { // MARKER: Handler
	var out HandlerOut
...
*/
func (_c Executor) TaskC(ctx context.Context, result string) (finalResult string, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{Result: result}, &out, _c.inFlow, _c.outFlow)
	return out.FinalResult, err // No trace
}

/*
Error creates and runs the Error workflow, blocking until termination.
*/
func (_c Executor) Error(ctx context.Context, trigger string) (finalResult string, status string, err error) { // MARKER: Error
	if _c.runner == nil {
		return finalResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out ErrorOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Error.URL(), ErrorIn{Trigger: trigger}, &out)
