...
*/
func (_c Executor) TaskB(ctx context.Context, attempts, target int) (succeeded bool, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{Attempts: attempts, Target: target}, &out, _c.inFlow, _c.outFlow)
	return out.Succeeded, err
}

/*
Handler creates and runs the Handler task.
*/
func (_c Executor) Handler(ctx context.Context, onErr *errors.TracedError, attempts int) (attemptsOut int, err error) { // MARKER: Handler
	var out HandlerOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Handler.Method, Handler.Route, HandlerIn{OnErr: onErr, Attempts: attempts}, &out, _c.inFlow, _c.outFlow)
	return out.AttemptsOut, err
}

/*
TaskC creates and runs the TaskC task.
*/
func (_c Executor) TaskC(ctx context.Context, attempts int) (finalAttempts int, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{Attempts: attempts}, &out, _c.inFlow, _c.outFlow)
	return out.FinalAttempts, err
}

/*
RetryLoop creates and runs the RetryLoop workflow.
*/
func (_c Executor) RetryLoop(ctx context.Context, target int) (finalAttempts int, status string, err error) { // MARKER: RetryLoop
	if _c.runner == nil {
		return finalAttempts, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out RetryLoopOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, RetryLoop.URL(), RetryLoopIn{Target: target}, &out)
	return out.FinalAttempts, status, err
}
