...
*/
func (_c Executor) TaskS(ctx context.Context, branch string) (branchOut string, err error) { // MARKER: TaskS
	var out TaskSOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskS.Method, TaskS.Route, TaskSIn{
		Branch: branch,
	}, &out, _c.inFlow, _c.outFlow)
	return out.BranchOut, err // No trace
}

/*
TaskA creates and runs the TaskA task.
*/
func (_c Executor) TaskA(ctx context.Context, path string) (pathOut string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{
		Path: path,
	}, &out, _c.inFlow, _c.outFlow)
	return out.PathOut, err // No trace
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, path string) (pathOut string, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{
		Path: path,
	}, &out, _c.inFlow, _c.outFlow)
	return out.PathOut, err // No trace
}
...
*/
func (_c Executor) TaskC(ctx context.Context, path string) (pathOut string, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{
		Path: path,
	}, &out, _c.inFlow, _c.outFlow)
	return out.PathOut, err // No trace
}
...
*/
func (_c Executor) TaskD(ctx context.Context, path string) (pathOut string, err error) { // MARKER: TaskD
	var out TaskDOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskD.Method, TaskD.Route, TaskDIn{
		Path: path,
	}, &out, _c.inFlow, _c.outFlow)
	return out.PathOut, err // No trace
}
...
/*
Alias creates and runs the Alias workflow, blocking until termination.
*/
func (_c Executor) Alias(ctx context.Context, branch string) (path string, status string, err error) { // MARKER: Alias
	if _c.runner == nil {
		return path, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out AliasOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Alias.URL(), AliasIn{
		Branch: branch,
	}, &out)
	return out.Path, status, err
}
