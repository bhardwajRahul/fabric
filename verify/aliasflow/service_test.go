...
func TestAliasflow_Alias(t *testing.T) { // MARKER: Alias
	...
	t.Run("history_distinguishes_b_and_bPrime_by_node_name", func(t *testing.T) {
		...
		flowKey, err := foremanClient.Create(ctx, aliasflowapi.Alias.URL(), aliasflowapi.AliasIn{Branch: ""})
		...
		flowKey, err = foremanClient.Create(ctx, aliasflowapi.Alias.URL(), aliasflowapi.AliasIn{Branch: "alt"})
		if !assert.NoError(err) {
			return
		}
		err = foremanClient.Start(ctx, flowKey)
