...
/*
TaskJ creates and runs the TaskJ task.
*/
func (_c Executor) TaskJ(ctx context.Context, normalResult string, innerResult int) (finalResult string, err error) { // MARKER: TaskJ
	var out TaskJOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskJ.Method, TaskJ.Route, TaskJIn{NormalResult: normalResult, InnerResult: innerResult}, &out, _c.inFlow, _c.outFlow)
...
*/
func (_c Executor) Inner(ctx context.Context) (innerResult int, status string, err error) { // MARKER: Inner
	if _c.runner == nil {
		return innerResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out InnerOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Inner.URL(), InnerIn{}, &out)
	return out.InnerResult, status, err
}

/*
Nested creates and runs the Nested workflow.
*/
func (_c Executor) Nested(ctx context.Context) (finalResult string, status string, err error) { // MARKER: Nested
	if _c.runner == nil {
		return finalResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out NestedOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Nested.URL(), NestedIn{}, &out)
