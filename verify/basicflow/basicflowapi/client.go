...
*/
func (_c Executor) TaskA(ctx context.Context) (path string, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{}, &out, _c.inFlow, _c.outFlow)
	return out.Path, err // No trace
}
...
/*
Basic creates and runs the Basic workflow, blocking until termination.
*/
func (_c Executor) Basic(ctx context.Context) (path string, status string, err error) { // MARKER: Basic
	if _c.runner == nil {
		return path, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out BasicOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Basic.URL(), BasicIn{}, &out)
