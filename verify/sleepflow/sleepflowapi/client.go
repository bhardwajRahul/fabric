...
*/
func (_c Executor) TaskA(ctx context.Context, sleepFor time.Duration) (sleepForOut time.Duration, err error) { // MARKER: TaskA
	var out TaskAOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskA.Method, TaskA.Route, TaskAIn{SleepFor: sleepFor}, &out, _c.inFlow, _c.outFlow)
	return out.SleepForOut, err
}

/*
TaskB creates and runs the TaskB task.
*/
func (_c Executor) TaskB(ctx context.Context, sleepFor time.Duration) (marked bool, err error) { // MARKER: TaskB
	var out TaskBOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskB.Method, TaskB.Route, TaskBIn{SleepFor: sleepFor}, &out, _c.inFlow, _c.outFlow)
...
*/
func (_c Executor) TaskC(ctx context.Context, marked bool) (completed bool, err error) { // MARKER: TaskC
	var out TaskCOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskC.Method, TaskC.Route, TaskCIn{Marked: marked}, &out, _c.inFlow, _c.outFlow)
	return out.Completed, err
}

/*
Delay creates and runs the Delay workflow.
*/
func (_c Executor) Delay(ctx context.Context, sleepFor time.Duration) (completed bool, status string, err error) { // MARKER: Delay
	if _c.runner == nil {
		return completed, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out DelayOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Delay.URL(), DelayIn{SleepFor: sleepFor}, &out)
	return out.Completed, status, err
}
