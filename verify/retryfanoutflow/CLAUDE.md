...
`flow.Retry(math.MaxInt32, 0, 0, 0)`; on success it contributes `[element+1]` as its `results` delta. Join is the
explicit `SetFanIn` target; the graph attaches `ReducerAppend` to `results` so each branch's
delta is appended at fan-in. The test asserts `results == [1..100]` for input `[0..99]`.

## Patterns exercised

- Dynamic fan-out (`graph.AddTransitionForEach`) over an integer array
- Explicit lineage fan-in (`graph.SetFanIn`)
- Explicit `SetReducer("results", ReducerAppend)` with single-element deltas
- Task-initiated infinite retry (`flow.Retry(math.MaxInt32, 0, 0, 0)`) inside a fan-out branch
- Deterministic fan-in ordering by `fan_out_ordinal`: despite random per-branch retry latency
  scrambling completion order, the appended `results` stays in input-array order. This is the
