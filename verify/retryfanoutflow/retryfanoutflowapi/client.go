...
/*
Enter creates and runs the Enter task.
*/
func (_c Executor) Enter(ctx context.Context, elements []int) (elementsOut []int, err error) { // MARKER: Enter
	var out EnterOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Enter.Method, Enter.Route, EnterIn{Elements: elements}, &out, _c.inFlow, _c.outFlow)
	return out.ElementsOut, err // No trace
}
...
func (_c Executor) RetryFanOut(ctx context.Context, elements []int) (listResult []int, status string, err error) { // MARKER: RetryFanOut
	...
	status, err = marshalWorkflow(ctx, _c.runner, RetryFanOut.URL(), RetryFanOutIn{Elements: elements}, &out)
