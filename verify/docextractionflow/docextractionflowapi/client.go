...
/*
ScanPDF creates and runs the ScanPDF task.
*/
func (_c Executor) ScanPDF(ctx context.Context, pdf []byte) (pageImages [][]byte, pageCount int, err error) { // MARKER: ScanPDF
	var out ScanPDFOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, ScanPDF.Method, ScanPDF.Route, ScanPDFIn{Pdf: pdf}, &out, _c.inFlow, _c.outFlow)
	return out.PageImages, out.PageCount, err // No trace
}

/*
IdentifyChunks creates and runs the IdentifyChunks task.
*/
func (_c Executor) IdentifyChunks(ctx context.Context, page []byte) (chunks []Rectangle, err error) { // MARKER: IdentifyChunks
	var out IdentifyChunksOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, IdentifyChunks.Method, IdentifyChunks.Route, IdentifyChunksIn{Page: page}, &out, _c.inFlow, _c.outFlow)
	return out.Chunks, err // No trace
}
...
/*
DocExtraction creates and runs the DocExtraction workflow, blocking until termination.
*/
func (_c Executor) DocExtraction(ctx context.Context, pdf []byte) (docTranscription string, pageCount int, status string, err error) { // MARKER: DocExtraction
	if _c.runner == nil {
		return docTranscription, pageCount, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out DocExtractionOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, DocExtraction.URL(), DocExtractionIn{Pdf: pdf}, &out)
	return out.DocTranscription, out.PageCount, status, err
}
