...
func TestFanoutflow_FanOut(t *testing.T) { // MARKER: FanOut
	t.Parallel()
	ctx := t.Context()
	...
	exec := fanoutflowapi.NewExecutor(tester).WithWorkflowRunner(foremanClient)
	...
	t.Run("static_fan_out_and_fan_in", func(t *testing.T) {
		...
		allMarked, status, err := exec.FanOut(ctx)
		assert.Expect(
			err, nil,
