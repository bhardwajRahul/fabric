...
/*
Record creates and runs the Record task.
*/
func (_c Executor) Record(ctx context.Context, tag string, delayMs int) (recorded bool, err error) { // MARKER: Record
	var out RecordOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, Record.Method, Record.Route, RecordIn{
		Tag:     tag,
		DelayMs: delayMs,
	}, &out, _c.inFlow, _c.outFlow)
	return out.Recorded, err // No trace
}

/*
Priority creates and runs the Priority workflow, blocking until termination.
*/
func (_c Executor) Priority(ctx context.Context, tag string, delayMs int) (recorded bool, status string, err error) { // MARKER: Priority
	if _c.runner == nil {
		return recorded, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out PriorityOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Priority.URL(), PriorityIn{
		Tag:     tag,
		DelayMs: delayMs,
	}, &out)
	return out.Recorded, status, err
}
