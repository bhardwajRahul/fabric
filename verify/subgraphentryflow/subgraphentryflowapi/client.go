...
/*
TaskInner creates and runs the TaskInner task.
*/
func (_c Executor) TaskInner(ctx context.Context) (innerResult string, err error) { // MARKER: TaskInner
	var out TaskInnerOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskInner.Method, TaskInner.Route, TaskInnerIn{}, &out, _c.inFlow, _c.outFlow)
	return out.InnerResult, err
}

/*
TaskTail creates and runs the TaskTail task.
*/
func (_c Executor) TaskTail(ctx context.Context, innerResult string) (finalResult string, err error) { // MARKER: TaskTail
	var out TaskTailOut
	err = marshalTask(ctx, _c.svc, _c.opts, _c.host, TaskTail.Method, TaskTail.Route, TaskTailIn{InnerResult: innerResult}, &out, _c.inFlow, _c.outFlow)
...
*/
func (_c Executor) Inner(ctx context.Context) (innerResult string, status string, err error) { // MARKER: Inner
	if _c.runner == nil {
...
/*
Tail creates and runs the Tail workflow.
*/
func (_c Executor) Tail(ctx context.Context, innerResult string) (finalResult string, status string, err error) { // MARKER: Tail
	if _c.runner == nil {
		return finalResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out TailOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Tail.URL(), TailIn{InnerResult: innerResult}, &out)
	return out.FinalResult, status, err
}

/*
Outer creates and runs the Outer workflow.
*/
func (_c Executor) Outer(ctx context.Context) (finalResult string, status string, err error) { // MARKER: Outer
	if _c.runner == nil {
		return finalResult, "", errors.New("workflow runner not set, use WithWorkflowRunner")
	}
	var out OuterOut
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, Outer.URL(), OuterIn{}, &out)
	return out.FinalResult, status, err
}
