...
func TestSubgraphentryflow_Outer(t *testing.T) { // MARKER: Outer
	t.Parallel()
	ctx := t.Context()
	...
	exec := subgraphentryflowapi.NewExecutor(tester).WithWorkflowRunner(foremanClient)
	...
	t.Run("subgraph_as_first_and_last_node", func(t *testing.T) {
		...
		final, status, err := exec.Outer(ctx)
		assert.Expect(
			err, nil,
