...
// emitClient renders *api/client.go for the service.
func emitClient(svc *service) ([]byte, error) {
	var sb strings.Builder
	sb.WriteString(licenseHeader)
	sb.WriteString("// Code generated by cmd/genservice. DO NOT EDIT.\n\n")
	fmt.Fprintf(&sb, "package %s\n\n", svc.apiPkg)
	writeImports(&sb, svc)
	sb.WriteString(clientVarBlock)
	sb.WriteString(clientPreamble)
	sb.WriteString(clientHelpers)
	for _, f := range svc.features {
		switch f.kind {
		case "Function":
			emitFunction(&sb, svc, f)
		case "Web":
			emitWeb(&sb, f)
		case "Task":
			emitTask(&sb, svc, f)
		case "Workflow":
			emitWorkflow(&sb, svc, f)
		case "OutboundEvent":
			emitOutboundEvent(&sb, svc, f)
		}
	}
	src := []byte(sb.String())
...
// writeImports emits the import block: the fixed base set plus any package referenced by an In/Out
// field type.
func writeImports(sb *strings.Builder, svc *service) {
	base := []string{
		"context",
		"encoding/json",
		"iter",
