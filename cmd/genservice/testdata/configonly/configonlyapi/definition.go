...
// Package configonlyapi is a fixture for an endpoint-less microservice: it declares only configs, no
// functions/webs/tasks/workflows/events. It pins the generated intermediate.go and mock_test.go for a
// microservice with no Subscribe surface, so the http/httpx/sub imports (intermediate) and the context
// import (mock_test) are emitted from the feature mix rather than leaking in via the api client.go.
