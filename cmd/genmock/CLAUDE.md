...
- The leading comment block (license header, banner, anything before the `package` clause) is reused verbatim from the existing `mock.go` if present. A fresh `mock.go` gets no header at all — the tool does not synthesize a license. Operators who want a header add it once to the file and the tool preserves it on every subsequent run.
