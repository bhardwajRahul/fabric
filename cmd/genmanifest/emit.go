...
// genmanifestMarker is the "code generated" line stamped at the top of every
// manifest so it is never mistaken for a hand-authored file.
const genmanifestMarker = "# Code generated by cmd/genmanifest. DO NOT EDIT."

// generatedLineRE matches a "code generated" marker comment so a prior one is
// stripped from the reused header and re-emitted exactly once (idempotency).
var generatedLineRE = regexp.MustCompile(`^#\s*Code generated .* DO NOT EDIT\.\s*$`)
...
func emit(m *Manifest, existingBytes []byte) []byte {
	...
	sb.WriteString(genmanifestMarker)
	sb.WriteString("\n\n")
...
func headerOf(existingBytes []byte) string {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(trim, "#") {
			if generatedLineRE.MatchString(trim) {
				// Drop a prior marker; emit re-adds it exactly once.
				continue
			}
			sb.WriteString(line)
			sawComment = true
...
func emitWebs(sb *strings.Builder, webs []Endpoint) {
	...
	for _, w := range webs {
		...
		if w.TimeBudget != "" {
			writeKV(sb, "    ", "timeBudget", w.TimeBudget)
		}
	}
}
...
func emitTasks(sb *strings.Builder, tasks []Endpoint) {
	...
	for _, t := range tasks {
		...
		if t.TimeBudget != "" {
			writeKV(sb, "    ", "timeBudget", t.TimeBudget)
		}
	}
}
...
// description, method, route, loadBalancing, requiredClaims, timeBudget.
func emitFunctionLike(sb *strings.Builder, e Endpoint) {
	...
	if e.TimeBudget != "" {
		writeKV(sb, "    ", "timeBudget", e.TimeBudget)
	}
}
