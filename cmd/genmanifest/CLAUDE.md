...
Source code is the single source of truth. `manifest.yaml` is a derived artifact, never hand-authored: every
regen overwrites it from the code (see "The manifest is overwritten - code is authoritative" below). Its primary
purpose is navigational. It gives coding agents a fast, accurate map of what a microservice exposes (configs,
metrics, endpoints, events) and what it depends on, so an agent can build a mental model of the system without
reading every source file. Because it is generated, it never drifts from the code it documents, which is what
makes it trustworthy as that map. A field belongs in the manifest when it helps an agent navigate the contract
(e.g. an endpoint's time budget as a latency hint); it does not belong there when it is ACL/topology data that
`cmd/gencreds` derives at deploy time (see "What this tool does NOT do").
...
Every regen rewrites `manifest.yaml` from scratch from the source files. Hand edits will be lost; the manifest is not meant to be edited. The leading comment block (license header, banner, anything before the first blank line) is reused verbatim from the existing file. The tool does not synthesize a license; operators who want one add it once and the tool preserves it on every subsequent run.

Every generated manifest always carries the line `# Code generated by cmd/genmanifest. DO NOT EDIT.` (emitted after the reused license header, if any, and before the `general:` section). It is the human signal that the file is machine-generated and must not be hand-edited. `headerOf` strips any prior marker line out of the captured header so it is re-emitted exactly once, keeping the generator idempotent (the `modifiedAt`-stability and `--check` guarantees still hold). One-time consequence: the first regen after this marker was introduced rewrites every existing manifest (to insert the marker and bump `modifiedAt`); subsequent regens are byte-stable.
