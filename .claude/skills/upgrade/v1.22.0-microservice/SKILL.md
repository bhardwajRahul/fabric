...
For each definition in the `events` section in `service.yaml`, perform the `microbus/add-outbound-event` skill.
...
For each definition in the `sinks` section in `service.yaml`, perform the `microbus/add-inbound-event` skill.
