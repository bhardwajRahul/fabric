---
name: upgrade-v1-22-0-microservice
user-invocable: false
description: Called by upgrade-v1-22-0. Upgrades a single microservice to v1.22.0.
---
...
For each definition in the `events` section in `service.yaml`, perform the `add-outbound-event` skill.
...
For each definition in the `sinks` section in `service.yaml`, perform the `add-inbound-event` skill.
