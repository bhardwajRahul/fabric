...
Describe **what the endpoint does and the effect it produces**, not who is expected to call it. The same endpoint may be hit by a browser, another microservice, a test, a curl script, or an LLM tool call - the description should read the same in every case. `"Renders a printable summary of the order"` is good; `"page used by the checkout flow"` is not. Naming the caller in the description leaks deployment context into the API contract and goes stale as soon as another caller shows up.
