---
name: add-function
description: TRIGGER when user asks to add, create, or modify an API endpoint, function, or RPC. Also when adding a route that accepts typed arguments and returns typed results. Edits definition.go and service.go; client.go, intermediate.go, mock.go, mock_test.go, and manifest.yaml are regenerated by cmd/genservice.
---

**CRITICAL**: Do NOT explore or analyze other microservices unless explicitly instructed to do so. The instructions in this skill are self-contained to this microservice.

**CRITICAL**: A functional endpoint is declared once, as a `define.Function` var in `<name>api/definition.go`, and implemented once, as a handler in `service.go`. The client stub, the `ToDo` interface entry, the subscription wiring, the marshaler, the mock, and the manifest entry are all GENERATED from `definition.go` by `cmd/genservice`. Never hand-write or hand-edit those files; add the declaration and run the generator.
