---
name: add-ticker
description: TRIGGER when user asks to add a recurring job, periodic task, scheduled operation, or ticker. Edits definition.go and service.go; intermediate.go, mock.go, mock_test.go, and manifest.yaml are regenerated by cmd/genservice.
---

**CRITICAL**: Do NOT explore or analyze other microservices unless explicitly instructed to do so. The instructions in this skill are self-contained to this microservice.

**CRITICAL**: A ticker is declared once, as a `define.Ticker` var in `<name>api/definition.go`, and implemented once, as a handler in `service.go`. The `ToDo` entry, the `StartTicker` wiring, the mock, and the manifest entry are all GENERATED from `definition.go` by `cmd/genservice`. Never hand-write or hand-edit those files; add the declaration and run the generator.
