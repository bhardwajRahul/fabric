...
description: TRIGGER when user asks to fire, emit, or publish an event that other services can subscribe to. Edits definition.go; the trigger, hook, response wrapper, and manifest entry are regenerated by cmd/genservice.
---

**CRITICAL**: Do NOT explore or analyze other microservices unless explicitly instructed to do so. The instructions in this skill are self-contained to this microservice.

**CRITICAL**: An outbound event is declared once, as a `define.OutboundEvent` var in `<name>api/definition.go`. The `MulticastTrigger`, the `Hook` that consumers subscribe through, the response wrapper, and the manifest entry are all GENERATED from `definition.go` by `cmd/genservice`. Never hand-write or hand-edit those files; add the declaration and run the generator. An outbound event has no handler in `service.go` - this service fires it; other services sink it.
