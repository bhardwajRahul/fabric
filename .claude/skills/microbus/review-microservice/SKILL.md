...
- Is `mock_test.go` present and unmodified? It is generated by `cmd/genmock` from the `ToDo` interface and should not be edited by hand. Flag any service that lacks it or whose `TestXxx_Mock` lives in `service_test.go` instead.
