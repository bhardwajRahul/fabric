...
- **Backward compatibility** - if the project is in a git repository, compare `*api/definition.go` (and any sibling type files) against the `main` branch. Flag removed or renamed exported types, fields, JSON tag names, `define.*` vars, routes, and signatures - all of these break downstream services that import the package. Skip this check if the project is not source-controlled or if the microservice was just created on this branch.
