...
The provider ships no typed model catalog and **no shipped default alias table**: provider model IDs rotate, so any maintained list would go stale. Instead `resolveModel` (in `service.go`) resolves a capability tier (`fast`/`default`/`smart`) through a `svc.modelAliases` table populated lazily from the live models-list API on first use. Each tier resolves to a Gemini floating `-latest` pointer (`fast` -> `gemini-flash-lite-latest`, `default` -> `gemini-flash-latest`, `smart` -> `gemini-pro-latest`), which self-updates to the current model with no code change. Only the cross-provider tiers are aliased - generic family words like `pro`/`flash` are deliberately **not** global aliases, since they would collide across providers under `"any"` resolution; a caller wanting a specific Gemini family uses its `gemini-`-prefixed floating-pointer name (`gemini-pro-latest`), which passes through by prefix. A `gemini-` prefixed name (a concrete id or a `-latest` pointer) passes through *without* the table, so a concrete `Turn` never depends on the models-list fetch and a newly released model works before it is listed. Anything unrecognized returns `""`. **Consequence of dropping defaults:** until a fetch succeeds, only concrete `gemini-` names resolve - a tier/family alias returns `""` (and `OnResolveProvider` answers `ok=false`) rather than a stale default. This is an accepted hard dependency: resolving an alias needs the same API and key as the chat call itself.

## Model Alias Refresh

The alias table is populated by `RefreshModels`, which fetches the Gemini models-list API (the base `ModelsURL`, `https://generativelanguage.googleapis.com/v1beta/models`) and rebuilds `svc.modelAliases`. It runs three ways: an eager warm at startup (launched from `OnStartup` via `svc.Go`, non-blocking), a 6h ticker, and a lazy fetch on the first alias resolve. The lazy path is `ensureAliases`: a **retryable "once"** - concurrent resolves are serialized by `fetchMu` and a populated table short-circuits, but a *failed* fetch leaves the table empty so the next call retries (a plain `sync.Once` is deliberately avoided so a wedged first fetch does not break alias resolution until the ticker). The table is swapped under `aliasMu` (an `RWMutex`; `resolveModel` reads under `RLock`).

`buildGeminiAliases` (pure, unit-tested) builds the table from the live list. Unlike the Claude and OpenAI providers there is no newest-by-timestamp selection to do: Gemini's per-family `-latest` pointers already float, so each alias maps to its family's pointer (from the `geminiLatestTargets` naming map), kept only when that pointer is present among the live text-generation models. `geminiTextModelIDs` filters the noisy list to text models (advertising `generateContent`, not named like a non-text family such as image/tts/embedding). An alias whose pointer is absent is dropped rather than guessed. A missing key is a no-op; a fetch failure is returned by `RefreshModels` (never swallowed) and logged by the framework from both its `svc.Go` warm-up and the 6h ticker, so a `401`/`403` surfaces in the log shortly after startup rather than only on first use.
