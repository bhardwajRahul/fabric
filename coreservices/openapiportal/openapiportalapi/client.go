...
// Endpoint routes.
const (
	RouteOfList = `//openapi:0` // MARKER: List
)
...
func (_c Client) List(ctx context.Context, method string, relativeURL string, body any) (res *http.Response, err error) { // MARKER: List
	...
	return _c.svc.Request(
		ctx,
		pub.Method(method),
		pub.URL(httpx.JoinHostAndPath(_c.host, RouteOfList)),
...
If it is of type url.Values, it is serialized as form data. All other types are serialized as JSON.
*/
func (_c MulticastClient) List(ctx context.Context, method string, relativeURL string, body any) <-chan *pub.Response { // MARKER: List
	if method == "" {
		method = "POST"
	}
