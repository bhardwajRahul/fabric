...
import (
	...
	"github.com/microbus-io/fabric/coreservices/openaillm/resources"
)
...
const (
	Hostname = openaillmapi.Hostname
	Version  = 1
)
...
func NewIntermediate(impl ToDo) *Intermediate {
	...
	svc.SetDescription(`The OpenAI LLM provider microservice implements the Turn endpoint for the OpenAI Chat Completions API.`)
	...
	svc.Subscribe(openaillmapi.Turn.Method, openaillmapi.Turn.Route, svc.doTurn) // MARKER: Turn
...
func (svc *Intermediate) doTurn(w http.ResponseWriter, r *http.Request) (err error) { // MARKER: Turn
	var in openaillmapi.TurnIn
	var out openaillmapi.TurnOut
	err = marshalFunction(w, r, openaillmapi.Turn.Route, &in, &out, func(_ any, _ any) error {
...
func (svc *Intermediate) doOpenAPI(w http.ResponseWriter, r *http.Request) (err error) {
	...
	endpoints := []*openapi.Endpoint{
		...
		{ // MARKER: Turn
			Type:    "function",
			Name:    "Turn",
			Method:  openaillmapi.Turn.Method,
			Route:   openaillmapi.Turn.Route,
			Summary: "Turn(messages []Message, tools []ToolDef) (completion *TurnCompletion)",
			Description: `Turn executes a single LLM turn using the OpenAI provider.
...
Output:
  - completion: completion is the LLM's response`,
			InputArgs:  openaillmapi.TurnIn{},
			OutputArgs: openaillmapi.TurnOut{},
		},
	}
