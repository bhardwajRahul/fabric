...
func TestOpenAILLM_OpenAPI(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
	...
	routes := []string{
		...
		openaillmapi.Turn.Route, // MARKER: Turn
	}
	for _, route := range routes {
		...
		t.Run("port_"+port, func(t *testing.T) {
			...
			res, err := tester.Request(
				ctx,
				pub.GET(httpx.JoinHostAndPath(openaillmapi.Hostname, ":"+port+"/openapi.json")),
			)
...
func TestOpenAILLM_Mock(t *testing.T) {
	t.Parallel()
	ctx := t.Context()
...
func TestOpenAILLM_Turn(t *testing.T) { // MARKER: Turn
	t.Parallel()
	ctx := t.Context()
	...
	tester := connector.New("tester.client")
	client := openaillmapi.NewClient(tester)
