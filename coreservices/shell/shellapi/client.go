...
// Get unpacks the return arguments of Execute.
func (_res *ExecuteResponse) Get() (exitCode int, stdout string, stderr string, err error) { // MARKER: Execute
	_d := _res.data.(*ExecuteOut)
	return _d.ExitCode, _d.Stdout, _d.Stderr, _res.err
}
...
*/
func (_c MulticastClient) Execute(ctx context.Context, cmd string, workDir string, stdin string, envars map[string]string) iter.Seq[*ExecuteResponse] { // MARKER: Execute
	_in := ExecuteIn{Cmd: cmd, WorkDir: workDir, Stdin: stdin, Envars: envars}
	_out := ExecuteOut{}
	_queue := marshalPublish(ctx, _c.svc, _c.opts, _c.host, Execute.Method, Execute.Route, &_in, &_out)
	return func(yield func(*ExecuteResponse) bool) {
		for _r := range _queue {
			_clone := _out
			_r.data = &_clone
			if !yield((*ExecuteResponse)(_r)) {
				return
			}
		}
	}
...
Output:
  ...
  - stderr: the standard error of the command
*/
func (_c Client) Execute(ctx context.Context, cmd string, workDir string, stdin string, envars map[string]string) (exitCode int, stdout string, stderr string, err error) { // MARKER: Execute
	_in := ExecuteIn{Cmd: cmd, WorkDir: workDir, Stdin: stdin, Envars: envars}
	_out := ExecuteOut{}
	err = marshalRequest(ctx, _c.svc, _c.opts, _c.host, Execute.Method, Execute.Route, &_in, &_out)
	return _out.ExitCode, _out.Stdout, _out.Stderr, err // No trace
}
