...
// Endpoint routes.
const (
	RouteOfCollect = `/collect` // MARKER: Collect
)

// Endpoint URLs.
var (
	URLOfCollect = httpx.JoinHostAndPath(Hostname, RouteOfCollect) // MARKER: Collect
)
...
func (_c Client) Collect(ctx context.Context, relativeURL string) (res *http.Response, err error) { // MARKER: Collect
	return _c.svc.Request(
		ctx,
		pub.Method("GET"),
		pub.URL(httpx.JoinHostAndPath(_c.host, RouteOfCollect)),
...
If a URL is provided, it is resolved relative to the URL of the endpoint.
*/
func (_c MulticastClient) Collect(ctx context.Context, relativeURL string) <-chan *pub.Response { // MARKER: Collect
