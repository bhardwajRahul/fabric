...
The provider ships no typed model catalog and **no shipped default alias table**: provider model IDs rotate every quarter, so any maintained list would go stale. Instead `resolveModel` (in `service.go`) resolves a capability tier (`fast`/`default`/`smart`) or a synthesized floating alias (`claude-opus-latest`/`claude-sonnet-latest`/`claude-haiku-latest`/`claude-fable-latest`) through a `svc.modelAliases` table populated lazily from the live models-list API on first use. Anthropic publishes no `-latest` pointer, so these are synthesized (each resolves to the newest concrete model of its family); they are namespaced under the `claude-` prefix on purpose - a bare family word like `opus`/`sonnet` is **not** a global alias, since it would collide across providers under `"any"` resolution. Only the neutral tiers are global. A concrete `claude-` id passes through *without* the table (a `claude-` prefix that does **not** end in `-latest`), so a concrete `Turn` never depends on the models-list fetch and a newly released model works before it is listed; a `claude-*-latest` name has no matching API model, so it falls to the table. Anything unrecognized returns `""`. **Consequence of dropping defaults:** until a fetch succeeds, only concrete `claude-` names resolve - a tier/family alias returns `""` (and `OnResolveProvider` answers `ok=false`) rather than a stale default. This is an accepted hard dependency: resolving an alias needs the same API and key as the chat call itself.

## Model Alias Refresh

The alias table is populated by `RefreshModels`, which fetches the Claude models-list API (`ModelsURL`, default `https://api.anthropic.com/v1/models`) and rebuilds `svc.modelAliases`. It runs three ways: an eager warm at startup (launched from `OnStartup` via `svc.Go`, non-blocking), a 6h ticker, and a lazy fetch on the first alias resolve. The lazy path is `ensureAliases`: a **retryable "once"** - concurrent resolves are serialized by `fetchMu` and a populated table short-circuits, but a *failed* fetch leaves the table empty so the next call retries. (A plain `sync.Once` is deliberately avoided: with no default fallback, a wedged first fetch would break alias resolution until the ticker.) The table is swapped under `aliasMu` (an `RWMutex`; `resolveModel` reads under `RLock`).

`buildClaudeAliases` (pure, unit-tested) builds the table from the live list: each family's synthesized `claude-<family>-latest` alias (`opus`/`sonnet`/`haiku`/`fable`) and the tier pointing at it (`haiku` -> `fast`, `sonnet` -> `default`, `opus` -> `smart`) resolves to the newest concrete model of that family, by `created_at`. Only the known families are mapped; a family absent from the list simply has no alias.

The same fetch also captures each model's `max_tokens` (its output cap) into `svc.modelMaxTokens` (`buildClaudeMaxTokens`, pure/unit-tested). Anthropic's Messages API **requires** `max_tokens` - unlike the other three providers, which omit the field and let the model use its own maximum - so `Turn` cannot leave it unset. It therefore defaults `max_tokens` to the resolved model's own output cap from that map (so responses are not artificially truncated), falling back to `claudeFallbackMaxTokens` (128k) only when the model's limit is not yet known (a concrete-passthrough model not in the list, or a call before the warm completes). A caller-supplied `options.MaxTokens` always wins. This provider does not stream, so a value above a model's streaming threshold will `400` until streaming is added. A missing key is a no-op; a fetch failure is returned by `RefreshModels` (never swallowed) and logged by the framework - its `svc.Go` warm-up and the 6h ticker both log a returned error - while staying non-fatal, so the last-known table keeps serving. A successful fetch doubles as early API-key validation: a `401`/`403` surfaces in the log shortly after startup rather than only on first use.
