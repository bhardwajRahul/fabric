...
`buildClaudeAliases` (pure, unit-tested) builds the table from the live list: each family's synthesized `claude-<family>-latest` alias (`opus`/`sonnet`/`haiku`/`fable`) and the tier pointing at it (`haiku` -> `fast`, `sonnet` -> `default`, `opus` -> `smart`) resolves to the newest concrete model of that family, by `created_at`. Only the known families are mapped; a family absent from the list simply has no alias. A missing key is a no-op; a fetch failure is returned by `RefreshModels` (never swallowed) and logged by the framework - its `svc.Go` warm-up and the 6h ticker both log a returned error - while staying non-fatal, so the last-known table keeps serving. A successful fetch doubles as early API-key validation: a `401`/`403` surfaces in the log shortly after startup rather than only on first use.
