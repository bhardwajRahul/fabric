...
func TestSpec_HTTPArguments(t *testing.T) {
	...
	assert.Error(err, "httpResponseBody can't be an input argument")
	err = yaml.Unmarshal([]byte("Hello(x int, httpStatusCode string) (ok bool)"), &sig)
	assert.Error(err, "httpStatusCode can't be an input argument")
	err = yaml.Unmarshal([]byte("Hello(x int, y string) (httpRequestBody bool)"), &sig)
	assert.Error(err, "httpRequestBody can't be an output argument")
	...
	assert.Error(err, "httpStatusCode must be of type int")
	err = yaml.Unmarshal([]byte("Hello(x int, y string) (httpResponseBody bool, z int, httpStatusCode int)"), &sig)
	assert.Error(err, "Output argument not allowed alongside httpResponseBody")
}
...
func TestSpec_TypedHandlers(t *testing.T) {
	...
	assert.Error(err, "Context type not allowed")
	err = yaml.Unmarshal([]byte("OnFunc(x int) (result int, err error)"), &sig)
	assert.Error(err, "Error type not allowed")
	err = yaml.Unmarshal([]byte("onFunc(x int) (y int)"), &sig)
	assert.Error(err, "Endpoint name must start with uppercase")
	err = yaml.Unmarshal([]byte("OnFunc(X int) (y int)"), &sig)
	assert.Error(err, "Arg name must start with lowercase")
	err = yaml.Unmarshal([]byte("OnFunc(x int) (Y int)"), &sig)
	assert.Error(err, "Arg name must start with lowercase")
	err = yaml.Unmarshal([]byte("OnFunc(x os.File) (y int)"), &sig)
	assert.Error(err, "Dot notation not allowed")
	err = yaml.Unmarshal([]byte("OnFunc(x Time) (x Duration)"), &sig)
	assert.Error(err, "Duplicate arg name")
