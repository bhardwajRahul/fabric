...
`Microbus` considers `error`s returned from microservice endpoints as exceptions, and as such it emphasizes their detection and surfacing. In addition to being logged as errors, these cross-microservice errors are also counted as a metric and visualized with Grafana dashboards, and noted in [distributed traces](../blocks/distrib-tracing.md). Consequently, it is ill-advised to use errors to return state that should not raise an alarm.
