...
`Microbus` considers errors returned from microservice endpoints as exceptions and emphasizes their detection and surfacing. In addition to being logged as errors, these cross-microservice errors are also counted by a [metric](../blocks/metrics.md), visualized in Grafana dashboards, and noted in [distributed traces](../blocks/distrib-tracing.md). Consequently, it is ill-advised to use errors to return state that should not raise an alarm.
...
func StandardHandler(w http.ResponseWriter, r *http.Request) {
	err := doSomething()
	if err != nil {
		logge.Error("failed to do something", "error", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	err = doSomethingElse()
	if err != nil {
		logge.Error("failed to do something else", "error", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
}
...
func FrameworkWrapperOfHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logger.Error("Handling request", "error", err)
...
Panics in Go that are not captured terminate the running process and for microservices that are expected to be always on, this is not a desirable outcome. To stop this from happening, the framework captures panics generated by user code and converts them to standard errors, which are then handled as such. The wrapper function introduced earlier comes in handy here.
...
func FrameworkWrapperOfHandler(w http.ResponseWriter, r *http.Request) {
	err := errors.CatchPanic(func() error {  // Convert panics to errors
		return UserCodeHandler(w, r)
	})
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		w.Write(fmt.Sprintf("%+v", err))
...
statusCode=400
trace=977496582a4ba7fa787012a94072892c
...
All microservices are ultimately web servers where it is common practice to return an appropriate HTTP status code along with an error. To that end, status codes can be associated with errors using either `errors.New` or `errors.Trace`. A `500 Internal Server Error` status code is associated with errors by default.

```go
func UserCodeHandler(w http.ResponseWriter, r *http.Request) error {
	params, err := parseRequest(r)
	if err != nil {
		return errors.Trace(err, http.BadRequest) // Wrap existing error and attach a status code
	}
	if obj, ok := m[params.Key]; !ok {
		return errors.New("record not found", http.StatusNotFound) // New error with status code
	}
	return nil
}
```
...
In addition to a status code, arbitrary name=value pairs can be associated with an error using either `errors.New` or `errors.Trace`. Properties are not part of the error's message and can be retrieved up the call stack in a structured way.

```go
func UserCodeHandler(w http.ResponseWriter, r *http.Request) error {
	id := r.URL.Query().Get("id")
	obj, ok err := loadObject(r.Context, id)
	if err != nil {
	...
	if !ok {
		return errors.Trace(
			err,
			http.StatusNotFound,
			"id", id, // Arbitrary name=value pair
		)
	}
...

{
	"err": {
		...
		"stack": [
			{
				"func": "connector.TestConnector_Error.func2",
				"file": "/src/github.com/microbus-io/fabric/connector/messaging_test.go",
				"line": 343
			},
			{
				"func": "connector.(*Connector).onRequest",
...
- Arbitrary name=value pairs can be associated with an error
