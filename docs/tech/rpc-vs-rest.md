...
`Create(httpRequestBody *Object) (id int, httpStatusCode int)` expects a `POST /objects` request with the object to be created in the body of the request. It returns the assigned `id` along with HTTP status code `201 Created`.
...
`Read(id int) (httpResponseBody *Object)` expects a `GET /objects/{id}` request with the ID of the object in the path. It returns the matching object.
...
`Update(id int, httpRequestBody *Object)` expects a `PUT /objects/{id}` request with the ID of the object in the path and the object to be created in the body of the request. It returns nothing.
...
`Delete(id int)` expects a `DELETE /objects/{id}` request with the ID of the object in the path. It returns nothing.
...
`ListAll(sortBy string, limit int, offset int) (httpResponseBody []*Object)` expects a `GET /objects?sortBy=created_at&limit=100&offset=0` request with sort order and cursor information as query arguments. It returns a list of objects.

```http
GET /objects?sortBy=created_at&limit=100&offset=0 HTTP/1.1
```
