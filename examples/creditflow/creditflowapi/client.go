...
func (_c Executor) IdentityVerification(ctx context.Context, applicantName string, ssn string, address string, phone string) (identityVerified bool, status string, err error) { // MARKER: IdentityVerification
	...
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, IdentityVerification.URL(), IdentityVerificationIn{
		ApplicantName: applicantName,
		SSN:           ssn,
		Address:       address,
		Phone:         phone,
...
func (_c Executor) CreditApproval(ctx context.Context, applicant Applicant) (approved bool, creditVerified bool, sumEmploymentFailures int, identityVerified bool, status string, err error) { // MARKER: CreditApproval
	...
	status, err = marshalWorkflow(ctx, _c.runner, _c.flowOptions, CreditApproval.URL(), CreditApprovalIn{
		Applicant: applicant,
	}, &out)
