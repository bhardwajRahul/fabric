...
- Directory: `examples/petstore`; Go package `petstore`; API package `petstoreapi`; module path
...
As a fixture inside the framework repo's `examples/`, this microservice is intentionally not added to
`main/main.go` (sibling examples are standalone). Validate with `go vet ./examples/petstore/...` and
`go build ./examples/petstore/...`.
